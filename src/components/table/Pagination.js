...
const Pagination = ({ previousPage, nextPage, canPreviousPage, canNextPage, pageIndex, pageOptions, gotoPage }) => {
    ...
    <ul className="pagination">
      ...
      {pageBtns.map((btn, index) => {
        return btn === null ? (
          ...
          <li key={`pagination_btn_${index}`} className={[btn === pageIndex ? "active" : ""].join(" ")} onClick={() => gotoPage(btn)}>
            {btn + 1}
          </li>
        );
      })}
      ...
      {pageOptions.length > 5 && (
        <div className="pagination_gotopage ">
          Jump to page
          <input
            className="ml-5"
            type="number"
